    ...
    def __call__(
        ...
        **kwargs,
        ...
        if mask is None:
            ...
            sliding_cache = cache[self.first_sliding_cache_idx] if self.first_sliding_cache_idx < len(cache) else None
            if sliding_cache is not None:
                sliding_window_mask = create_attention_mask(h, [sliding_cache])
            else:
                from mlx_lm.models.base import create_causal_mask
                sliding_window_mask = create_causal_mask(h.shape[1], 0, window_size=self.window_size)
